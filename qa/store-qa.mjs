...
for(const p of products){
  ...
  if(p.status==="published"&&p.lifecycle?.stage!=="PUBLISHED") fail(p.slug+": published status without PUBLISHED lifecycle");
...
console.log(`Store QA passed: ${products.length} products, ${files.filter(f=>f.endsWith(".html")).length} HTML pages checked.`);
